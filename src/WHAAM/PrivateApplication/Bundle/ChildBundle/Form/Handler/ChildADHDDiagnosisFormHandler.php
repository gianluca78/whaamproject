...
class ChildADHDDiagnosisFormHandler {
    ...
    public function handle(FormInterface $form, Request $request, $message)
    {
        ...
        $validChildADHDDiagnosis = $form->getData();
        $this->createChildADHDDiagnosis($validChildADHDDiagnosis, $message);

        return $validChildADHDDiagnosis;
    }

    public function createChildADHDDiagnosis(ChildADHDDiagnosis $childADHDDiagnosis, $message)
    {
        $successMessage = $this->translator->trans('success', array(), 'flash_messages') . ' ' .
            $this->translator->trans($message, array(), 'flash_messages');

        $this->entityManager->persist($childADHDDiagnosis);
