...
class ChildSchoolInformationFormHandler {
    ...
    public function handle(FormInterface $form, Request $request, $message)
    {
        ...
        $validSchoolInformation = $form->getData();
        $this->createSchoolInformation($validSchoolInformation, $message);

        return $validSchoolInformation;
    }

    public function createSchoolInformation(ChildSchoolInformation $childSchoolInformation, $message)
    {
        $successMessage = $this->translator->trans('success', array(), 'flash_messages') . ' ' .
            $this->translator->trans($message, array(), 'flash_messages');

        $this->entityManager->persist($childSchoolInformation);
