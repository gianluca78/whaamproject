...
class ChildDisciplineReferralFormHandler {
    ...
    public function handle(FormInterface $form, Request $request, $message)
    {
        ...
        $validChildDisciplineReferral = $form->getData();
        $this->createChildDisciplineReferral($validChildDisciplineReferral, $message);

        return $validChildDisciplineReferral;
    }

    public function createChildDisciplineReferral(ChildDisciplineReferral $childDisciplineReferral, $message)
    {
        $successMessage = $this->translator->trans('success', array(), 'flash_messages') . ' ' .
            $this->translator->trans($message, array(), 'flash_messages');

        $this->entityManager->persist($childDisciplineReferral);
