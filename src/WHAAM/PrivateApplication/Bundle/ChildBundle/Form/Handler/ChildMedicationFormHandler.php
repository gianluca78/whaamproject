...
class ChildMedicationFormHandler {
    ...
    public function handle(FormInterface $form, Request $request, $message)
    {
        ...
        $validMedication = $form->getData();
        $this->createMedication($validMedication, $message);

        return $validMedication;
    }

    public function createMedication(ChildMedication $childMedication, $message)
    {
        $successMessage = $this->translator->trans('success', array(), 'flash_messages') . ' ' .
            $this->translator->trans($message, array(), 'flash_messages');

        $this->entityManager->persist($childMedication);
