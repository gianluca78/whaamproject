...
class ChildBehaviorFormHandler {
    ...
    public function handle(FormInterface $form, Request $request, $message)
    {
        ...
        $validChildBehavior = $form->getData();
        $this->createChildBehavior($validChildBehavior, $message);

        return $validChildBehavior;
    }

    public function createChildBehavior(ChildBehavior $childBehavior, $message)
    {
        $successMessage = $this->translator->trans('success', array(), 'flash_messages') . ' ' .
            $this->translator->trans($message, array(), 'flash_messages');

        $this->entityManager->persist($childBehavior);
